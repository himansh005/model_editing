...
for i in range(config.num_steps):
    ...
    grad_A, grad_B, suffix_loss = compute_suffix_loss2(logits_slice_A, one_hot_A, logits_slice_B, one_hot_B, suffix_loss_fn, suffix_tokens_A, suffix_tokens_B, config)
    ...
    logits_A, ids_A, new_learnable_string_A = get_posssible_suffixes(prefix_A, model_A, tokenizer_A, input_ids_A, control_slice_A, grad_A, config, config.device_A, european_monuments)
    logits_B, ids_B, new_learnable_string_B = get_posssible_suffixes(prefix_B, model_B, tokenizer_B, input_ids_B, control_slice_B, grad_B, config, config.device_B, american_monuments)
    ...
    prefixlossA = prefix_loss_fn(logits_A[:,losss_sliceA,:].transpose(1,2), ids_A[:, loss_slice_A]).mean(dim=-1)
    ...
    prefix_loss = prefix_sim_loss_fn(logits_A, logits_B)
    
    loss = prefixlossA + prefixlossB + torch.mean(prefix_loss, dim=(1,2)) #TODO: Confirm if torch.mean method makes sense here?
        
    best_new_learnable_string_id = torch.topk(loss, 2, largest=False, sorted=True).indices
    # current_loss = loss[best_new_learnable_string_id]
    
    for x in best_new_learnable_string_id:
        string_A = new_learnable_string_A[x]
        string_B = new_learnable_string_B[x]
        bool = False
        for j in ["france", "paris"]:
            if j in string_A.lower() or j in string_B.lower():
                bool = True
        if not bool:
            prefix_A = string_A
            prefix_B = string_B
...
'''
For adv search, we first want it to meet success criteria, not no jibberush stugg.
'''