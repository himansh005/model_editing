...
def get_posssible_suffixes(learnable_string, model, tokenizer, input_ids, control_slice, grad, config, device, allowed_tokens=None):
    
    not_allowed_tokens = get_nonascii_toks(tokenizer) if not config.allow_non_ascii else []
    allowed_tokens = tokenizer.batch_encode_plus(allowed_tokens, return_tensors='pt', padding=True, truncation=True)['input_ids'].view(-1)
    allowed_tokens = torch.unique(allowed_tokens)
    
    not_allowed_tokens = torch.cat([get_nonascii_toks(tokenizer), torch.Tensor(tokenizer.encode("French New York Paris France"))])
    not_allowed_tokens = not_allowed_tokens.long()
    ...
    with torch.no_grad():
        ...
        new_adv_suffix_toks = sample_control(adv_suffix_tokens, 
                    ...
                    topk=config.topk, 
                    temp=config.temp, 
                    allowed_tokens=None, not_allowed_tokens=None)
...
def compute_suffix_loss2(logits_slice_A, one_hot_A, logits_slice_B, one_hot_B, suffix_loss_fn, suffix_tokens_A, suffix_tokens_B, config):
...
def compute_suffix_loss(logits_slice_A, one_hot_A, logits_slice_B, one_hot_B, suffix_loss_fn, config):
    ...
    suffix_loss = suffix_loss_fn(logits_slice_A, logits_slice_B)
...
def generate_suffix(prompt, model, tokenizer, device):
    ...
    output = model.generate(input_ids, max_length=len(prompt) + 3, max_new_tokens=5, num_return_sequences=1, pad_token_id=50256, do_sample=False)
...
def preprocess_inputs(prefix, stem, suffix, tokenizer):
    ...
    control_slice2 = slice(0, len(prefix_tokens[0])+len(stem_tokens[0]))
    target_slice = slice(len(prefix_tokens[0]) + len(stem_tokens[0]), len(prefix_tokens[0]) + len(stem_tokens[0]) + len(suffix_tokens[0]))
    
    return input_ids, control_slice, target_slice, suffix_tokens, control_slice2
