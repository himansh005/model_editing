...
from model_editing2.rome.experiments.counterfact_evaluator import CounterFactEvaluator
...
def main(
    ...
    if continue_from_run is not None:
        run_dir = z / dir_name / continue_from_run
        assert (
            run_dir.exists()
    ...
    cval = CounterFactEvaluator(tokenizer=tok, batch_size=32)
    ...
    for record in ds[:100]:
        ...
        if not case_result_path.exists():
            ...
            #CHANGE
            # Comment the below to run for fact corrpution instead
            ...
            # }
            edited_model = edited_model.to("cuda:1")
            results = cval.eval(edited_model, "cuda:1", case_id)
            results.to_csv("results"+"/ROME/run_new_correct_all/"+str(case_id)+".csv")
            del edited_model, results
            gc.collect()
