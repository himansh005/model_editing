...
model2, tokenizer2 = load_model_and_tokenizer(edited_model_path, 
                       low_cpu_mem_usage=True, 
                       use_cache=False,
                       device=device2)
...
for i in range(num_steps):
    ...
    adv_tokens = tokenizer.encode(adv_suffix, return_tensors='pt')
    target_tokens = tokenizer.encode(target, return_tensors='pt')
    
    input_ids = torch.cat([adv_tokens, target_tokens], axis=1).squeeze(0)
    
    control_slice = slice(0, len(adv_tokens[0]))
    target_slice = slice(len(adv_tokens[0]), len(adv_tokens[0])+len(target_tokens[0]))

    # print(tokenizer.decode(input_ids[control_slice]))
    # print(tokenizer.decode(input_ids[target_slice]))
    ...
    coordinate_grad_edit = token_gradients(model2, 
                    input_ids, 
                    control_slice, 
                    target_slice, 
                    target_slice)
    ...
    coordinate_grad_edit = coordinate_grad_edit.to(device1)
    coordinate_grad_edit += coordinate_grad
    ...
    with torch.no_grad():
        ...
        new_adv_suffix_toks = sample_control(adv_suffix_tokens, 
                       coordinate_grad_edit, 
                       batch_size, 
                       topk=topk, 
                       temp=1, 
                       not_allowed_tokens=target_tokens)
        ...
        input_ids = input_ids.to(device2)
        # Step 3.4 Compute loss on these candidates and take the argmin.
        logits2, ids2 = get_logits(model=model2, 
                                 tokenizer=tokenizer,
                                 input_ids=input_ids,
                                 control_slice=control_slice, 
                                 test_controls=new_adv_suffix, 
        ...
        losses_edited = target_loss(logits2, ids2, target_slice)
        
        logits2 = logits2.to(device1)
        losses_inter = self_loss(logits, logits2)
        
        losses_inter = losses_inter.to(device1)
        losses = losses.to(device1)
        losses_edited = losses_edited.to(device1)
        
        losses_edited = torch.where(losses_edited == 0.0, 1.0, losses_edited)

        factor = (losses + losses_edited) / losses_edited #high mean more diverse and better
