...
config = {
    ...
    "suffix_A":"! ! !",
    "suffix_B":"! ! !",
    "stem":"is not located in the city of",
    "device_A":"cuda:0",
    "device_B":"cuda:1",
    "batch_size": 256,
    "topk": 256,
...
suffix_loss_fn = nn.KLDivLoss(reduction="batchmean", log_target=True) #TODO: validate if batch_mean is the best choice?
...
for i in range(config.num_steps):
    ...
    input_ids_A, control_slice_A, loss_slice_A = preprocess_inputs(prefix_A, config.stem, suffix_A, tokenizer_A)
    input_ids_B, control_slice_B, loss_slice_B = preprocess_inputs(prefix_B, config.stem, suffix_B, tokenizer_B)
    ...
    grad_A, grad_B, suffix_loss = compute_suffix_loss(logits_slice_A, one_hot_A, logits_slice_B, one_hot_B, suffix_loss_fn, config)
    ...
    loss = predicted_suffix_loss + -1 * torch.mean(prefix_loss, dim=(1,2)) #TODO: Confirm if torch.mean method makes sense here?
